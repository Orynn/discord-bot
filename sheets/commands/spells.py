...
def register_spell_commands(sheet_group: Group) -> None:
    ...
    async def sheet_spells_group(ctx: Context, member: discord.Member | None = None) -> None:
    ...
    async def sheet_spells_remove(
        ...
        *,
        ...
        if slug is None:
            homebrew_match = next(
                (spell_name for spell_name in sheet.homebrew_spells if spell_name.lower() == query),
                None,
            )
